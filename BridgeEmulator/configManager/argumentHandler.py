...
def parse_arguments():
    argumentDict = {"BIND_IP": '', "HOST_IP": '', "HTTP_PORT": '', "HTTPS_PORT": '', "FULLMAC": '', "MAC": '', "DEBUG": False, "DOCKER": False,
                    "scanOnHostIP": False,"noLinkButton": False, "noServeHttps": False}
    ...
    ap.add_argument("--ip-range", help="Depricated use webui, Set IP range for light discovery. Format: <START_IP>,<STOP_IP>", type=str)
    ap.add_argument("--sub-ip-range", help="Depricated use webui, Set SUB IP range for light discovery. Format: <START_IP>,<STOP_IP>", type=str)
    ap.add_argument("--scan-on-host-ip", action='store_true', help="Scan the local IP address when discovering new lights")
    ap.add_argument("--deconz", help="Depricated use webui, Provide the IP address of your Deconz host. 127.0.0.1 by default.", type=str)
    ...
    ap.add_argument("--disable-online-discover", help="Depricated use webui, Disable Online and Remote API functions")
    ...
    if args.sub_ip_range or get_environment_variable('SUB_IP_RANGE') or get_environment_variable('SUB_IP_RANGE_START') or get_environment_variable('SUB_IP_RANGE_END'):
        logging.warn("IP range is depricated in commandline and not active, please setup via webui")
