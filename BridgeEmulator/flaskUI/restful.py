    ...
    def post(self):
        ...
        if "devicetype" in postDict:
            ...
            if last_button_press + 30 >= datetime.now().timestamp(): # 30 sec offset
                ...
                bridgeConfig["apiUsers"][username] = ApiUser(username, postDict["devicetype"], client_key)
        ...
        if resource == "groups":
            if "type" in postDict:
                if postDict["type"] == "Zone":
                    v2Resource = "zone"
                    bridgeConfig[resource][new_object_id] = Group(postDict)
                elif postDict["type"] == "Room":
                    v2Resource = "room"
                    bridgeConfig[resource][new_object_id] = Group(postDict)
                elif postDict["type"] == "Entertainment":
                    v2Resource = "entertainment_configuration"
                    bridgeConfig[resource][new_object_id] = EntertainmentConfiguration(postDict)
            ...
            bridgeConfig[resource][new_object_id] = Scene(postDict)
            ...
            bridgeConfig[resource][new_object_id] = Rule(postDict)
        elif resource == "resourcelinks":
            bridgeConfig[resource][new_object_id] = ResourceLink(postDict)
        elif resource == "sensors":
            v2Resource = "device"
            bridgeConfig[resource][new_object_id] = Sensor(postDict)
        elif resource == "schedules":
            bridgeConfig[resource][new_object_id] = Schedule(postDict)
