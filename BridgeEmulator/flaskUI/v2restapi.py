...
def _identifyLight(device):
    """Run a one-shot visual identify without saving light configuration.

    Zigbee2MQTT devices that explicitly expose a native ``blink`` effect use
    it. Every other protocol stays on diyHue's existing Hue ``select`` alert
    fallback. The helper sends exactly one transient command and does not
    write scenes, metadata, or the persistent bridge config.
    """
    light = device.firstElement() if hasattr(device, "firstElement") else device
    protocol_config = getattr(light, "protocol_cfg", {})
    capabilities = (
        protocol_config.get("z2m_capabilities", {})
        if isinstance(protocol_config, dict)
        else {}
    )
    effects = (
        capabilities.get("effect_list", [])
        if isinstance(capabilities, dict)
        else []
    )
    command = (
        {"effect": "blink"}
        if getattr(light, "protocol", None) == "mqtt" and "blink" in effects
        else {"alert": "select"}
    )
    light.setV1State(command)
    logging.info(
        "identify light=%s protocol=%s action=%s",
