...
def saveConfig(filename='config.json'):
    with open(cwd + '/../' + filename, 'w', encoding="utf-8") as fp:
        json.dump(Globals.bridge_config, fp, sort_keys=True, indent=4, separators=(',', ': '))
    if Globals.docker:
...
def loadConfig():
    try:
        ...
        if os.path.exists(path):
            ...
            return Globals.bridge_config
        else:
            logging.info("Config not found, creating new config from default settings")
            Globals.bridge_config = load_config(cwd + '/default-config.json')
            saveConfig(Globals.bridge_config)
            return Globals.bridge_config
    except Exception:
