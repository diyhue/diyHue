...
def isMotionAwareCandidateDevice(device):
    ...
    A candidate must opt in through an adapter that can actually supply the
    radio measurement contract.  A Zigbee2MQTT ``linkquality`` value is a
    routing/link metric, not a supported MotionAware measurement, so merely
    being a Zigbee light must never make a device eligible.

    Reachability is dynamic state, not capability.  Keeping the service on an
    unreachable eligible device preserves references for an existing area; the
    create validator still filters unreachable devices via
