    ...
    for condition in Globals.bridge_config["rules"][rule]["conditions"]:
        try:
            ...
            if condition["operator"] == "eq":
                ...
                if device_found == False or not Globals.dxState[url_pices[1]][url_pices[2]][url_pices[3]][url_pices[4]] == current_time:
                    return [False, 0]
    ...
    if rule_state[0]: #if all conditions are meet again
        ...
        for action in Globals.bridge_config["rules"][rule]["actions"]:
            if action["method"] == "POST":
                requests.post("http://localhost/api/" + Globals.bridge_config["rules"][rule]["owner"] + action["address"], json=action["body"], timeout=5)
            elif action["method"] == "PUT":
                requests.put("http://localhost/api/" + Globals.bridge_config["rules"][rule]["owner"] + action["address"], json=action["body"], timeout=5)
