...
def scanDeconz():
    ...
    if not bridge_config["emulator"]["deconz"]["enabled"]:
        if "username" not in bridge_config["emulator"]["deconz"]:
            try:
                registration = json.loads(sendRequest("http://" + deconz_ip + ":" + str(bridge_config["emulator"]["deconz"]["port"]) + "/api", "POST", "{\"username\": \"283145a4e198cc6535\", \"devicetype\":\"Hue Emulator\"}"))
            except:
                logging.info("registration fail, is the link button pressed?")
                if "websocketport" in bridge_config["emulator"]["deconz"]:
                    del bridge_config["emulator"]["deconz"]["websocketport"]
                return
            if "success" in registration[0]:
                bridge_config["emulator"]["deconz"]["username"] = registration[0]["success"]["username"]
                bridge_config["emulator"]["deconz"]["enabled"] = True
    elif "username" in bridge_config["emulator"]["deconz"]:
        deconz_config = json.loads(sendRequest("http://" + deconz_ip + ":" + str(bridge_config["emulator"]["deconz"]["port"]) + "/api/" + bridge_config["emulator"]["deconz"]["username"] + "/config", "GET", "{}"))
        bridge_config["emulator"]["deconz"]["websocketport"] = deconz_config["websocketport"]

        #lights
        deconz_lights = json.loads(sendRequest("http://" + deconz_ip + ":" + str(bridge_config["emulator"]["deconz"]["port"]) + "/api/" + bridge_config["emulator"]["deconz"]["username"] + "/lights", "GET", "{}"))
        for light in deconz_lights:
            if light not in bridge_config["emulator"]["deconz"]["lights"] and "modelid" in deconz_lights[light]:
                new_light_id = nextFreeId(bridge_config, "lights")
                logging.info("register new light " + new_light_id)
                bridge_config["lights"][new_light_id] = deconz_lights[light]
                bridge_config["lights_address"][new_light_id] = {"username": bridge_config["emulator"]["deconz"]["username"], "light_id": light, "ip": deconz_ip + ":" + str(bridge_config["emulator"]["deconz"]["port"]), "protocol": "deconz"}
                bridge_config["emulator"]["deconz"]["lights"][light] = {"bridgeid": new_light_id, "modelid": deconz_lights[light]["modelid"], "type": deconz_lights[light]["type"]}

        #sensors
        deconz_sensors = json.loads(sendRequest("http://" + deconz_ip + ":" + str(bridge_config["emulator"]["deconz"]["port"]) + "/api/" + bridge_config["emulator"]["deconz"]["username"] + "/sensors", "GET", "{}"))
        for sensor in deconz_sensors:
            if sensor not in bridge_config["emulator"]["deconz"]["sensors"] and "modelid" in deconz_sensors[sensor]:
                ...
                if deconz_sensors[sensor]["modelid"] in ["TRADFRI remote control", "TRADFRI wireless dimmer"]:
                    ...
                    bridge_config["emulator"]["deconz"]["sensors"][sensor] = {"bridgeid": new_sensor_id, "modelid": deconz_sensors[sensor]["modelid"], "type": deconz_sensors[sensor]["type"]}
                    ...
                    bridge_config["emulator"]["deconz"]["sensors"][sensor] = {"bridgeid": newMotionSensorId, "triggered": False, "modelid": deconz_sensors[sensor]["modelid"], "type": deconz_sensors[sensor]["type"], "lightsensor": "internal"}
                    ...
                    newMotionSensorId = addHueMotionSensor("", deconz_sensors[sensor]["name"])
                    bridge_config["emulator"]["deconz"]["sensors"][sensor] = {"bridgeid": newMotionSensorId, "triggered": False, "modelid": deconz_sensors[sensor]["modelid"], "type": deconz_sensors[sensor]["type"], "lightsensor": "astral"}
                    ...
                    newMotionSensorId = addHueMotionSensor("", deconz_sensors[sensor]["name"])
                    bridge_config["emulator"]["deconz"]["sensors"][sensor] = {"bridgeid": newMotionSensorId, "triggered": False, "modelid": deconz_sensors[sensor]["modelid"], "type": deconz_sensors[sensor]["type"]}
                else:
                    bridge_config["sensors"][new_sensor_id] = deconz_sensors[sensor]
                    bridge_config["emulator"]["deconz"]["sensors"][sensor] = {"bridgeid": new_sensor_id, "modelid": deconz_sensors[sensor]["modelid"], "type": deconz_sensors[sensor]["type"]}

            else: #temporary patch for config compatibility with new release
                bridge_config["emulator"]["deconz"]["sensors"][sensor]["modelid"] = deconz_sensors[sensor]["modelid"]
                bridge_config["emulator"]["deconz"]["sensors"][sensor]["type"] = deconz_sensors[sensor]["type"]
        generateDxState()
...
def websocketClient():
    ...
    if "websocketport" not in bridge_config["emulator"]["deconz"]:
        return
        ...
        def received_message(self, m):
            ...
            try:
                if message["r"] == "sensors":
                    bridge_sensor_id = bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["bridgeid"]
                    if "state" in message and bridge_config["sensors"][bridge_sensor_id]["config"]["on"]:

                        #change codes for emulated hue Switches
                        if "hueType" in bridge_config["emulator"]["deconz"]["sensors"][message["id"]]:
                            ...
                            message["state"]["buttonevent"] = rewriteDict[bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["hueType"]][message["state"]["buttonevent"]]
                        #end change codes for emulated hue Switches

                        #convert tradfri motion sensor notification to look like Hue Motion Sensor
                        if message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["modelid"] == "TRADFRI motion sensor":
                            ...
                            for sensor in bridge_config["sensors"].keys():
                                if bridge_config["sensors"][sensor]["type"] == "ZLLLightLevel" and bridge_config["sensors"][sensor]["uniqueid"] == bridge_config["sensors"][bridge_sensor_id]["uniqueid"][:-1] + "0":
                                    light_sensor = sensor
                                    break
                            if bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["lightsensor"] == "none":
                                message["state"].update({"dark": True})
                            elif bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["lightsensor"] == "astral":
                                message["state"]["dark"] = not bridge_config["sensors"]["1"]["state"]["daylight"]

                            elif bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["lightsensor"] == "combined":
                        ...
                        if message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["modelid"] == "lumi.sensor_motion":
                            for sensor in bridge_config["sensors"].keys():
                                if bridge_config["sensors"][sensor]["type"] == "ZLLLightLevel" and bridge_config["sensors"][sensor]["uniqueid"] == bridge_config["sensors"][bridge_sensor_id]["uniqueid"][:-1] + "0":
                                    light_sensor = sensor
                                    break
                        ...
                        if message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["modelid"] == "lumi.sensor_motion.aq2" and message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["type"] == "ZHALightLevel":
                        ...
                        if message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["modelid"] == "lumi.vibration.aq1":
                            #find the light sensor id
                            light_sensor = "0"
                            for sensor in bridge_config["sensors"].keys():
                                if bridge_config["sensors"][sensor]["type"] == "ZLLLightLevel" and bridge_config["sensors"][sensor]["uniqueid"] == bridge_config["sensors"][bridge_sensor_id]["uniqueid"][:-1] + "0":
                        ...
                        if "buttonevent" in message["state"] and bridge_config["emulator"]["deconz"]["sensors"][message["id"]]["modelid"] in ["TRADFRI remote control","RWL021","TRADFRI on/off switch"]:
                    ...
                    bridge_light_id = bridge_config["emulator"]["deconz"]["lights"][message["id"]]["bridgeid"]
    ...
    try:
        deconz_ip = configManager.runtimeConfig.arg["DECONZ"]
        ws = EchoClient('ws://' + deconz_ip + ':' + str(bridge_config["emulator"]["deconz"]["websocketport"]))
        ws.connect()
        ws.run_forever()
