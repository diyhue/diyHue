...
from services.updateManager import versionCheck, githubCheck, githubInstall
...
def runScheduler():
    while True:
        ...
        if (datetime.now().strftime("T%H%M:%S") == bridgeConfig["swupdate2"]["autoinstall"]["updatetime"]) and bridgeConfig["swupdate2"]["autoinstall"]["on"] == True:
            githubInstall()
