...
#define DFL_PSK "321c0c2ebfa7361e55461095b2f5f9db"
...
int main(int argc, char *argv[])
{
    ...
    if (opt.async_operations[0] != '-')
    {
        ...
        ssl_async_keys.inject_error = (opt.async_private_error < 0 ? -opt.async_private_error : opt.async_private_error);
...
data_exchange:
    ...
    else /* Not stream, so datagram */
    {
        ...
        mbedtls_printf(" version: %d.%d\n", buf[9], buf[10]);
        mbedtls_printf(" sequence no: %d\n", buf[11]);
        mbedtls_printf(" color mode: %x\n", buf[14]);
        */

        ret = 0;

        /*char data[256] = "curl -X PUT 127.0.0.1/entertainment -d '{\"colorspace\":";

        if (buf[14] == 0x00)
        {
            strcat(data, "\"rgb\",\"data\":{");
        }
        else if (buf[14] == 0x01)
        {
            strcat(data, "\"xy\",\"data\":{");
        ...
        {
            //mbedtls_printf(" type: %x\n", buf[i]);
            //mbedtls_printf(" id: %x %x\n", buf[i + 1], buf[i + 2]);
            if (i != 16 && (i + 9) > len)
            {
                strcat(data, ",");
            }
            ...
            int id = a + b;
            char idchar[10];
            memset(idchar, '\0', sizeof(char) * 10);
            sprintf(idchar, "%d", id);
            ...
            char colValChar[5];
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 3]);
            strcat(data, colValChar);
            strcat(data, ",");
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 4]);
            strcat(data, colValChar);
            strcat(data, ",");
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 5]);
            strcat(data, colValChar);
            strcat(data, ",");
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 6]);
            strcat(data, colValChar);
            strcat(data, ",");
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 7]);
            strcat(data, colValChar);
            strcat(data, ",");
            memset(colValChar, '\0', sizeof(char) * 5);

            sprintf(colValChar, "%d", (int)buf[i + 8]);
            strcat(data, colValChar);
            memset(colValChar, '\0', sizeof(char) * 5);

            strcat(data, "]");

            //mbedtls_printf(" color val: %x %x, %x %x, %x %x\n", buf[i + 3], buf[i + 4], buf[i + 5], buf[i + 6], buf[i + 7], buf[i + 8]);

            i = i + 9;
        }

        strcat(data, "}}' --tcp-nodelay");
        printf("%s \n", data);
