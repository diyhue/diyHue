...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($data['scene'])) {
        ...
        while ($row_scene = mysqli_fetch_assoc($query_scene)) {
            ...
            $update_string         = 'UPDATE lights SET state = ' . (int) $row_scene['state'];
            if ($row_scene['xy'] != '') {
                $update_string_actions = ', bri = ' . $row_scene['bri'] . ", xy = '" . $row_scene['xy'] . "', colormode = 'xy' ";
                $curl_arguments['xy']  = json_decode($row_scene['xy'], true);
            } elseif ($row_scene['ct'] != 0) {
                $update_string_actions = ', bri = ' . $row_scene['bri'] . ', ct = ' . $row_scene['ct'] . ", colormode = 'ct' ";
    ...
    } elseif (isset($data['bri_inc'])) {
        $query_lights = mysqli_query($con, 'SELECT lights FROM groups WHERE id = ' . $url['4'] . ';');
        $row_lights   = mysqli_fetch_assoc($query_lights);
        #error_log('group on :' . $row_lights['lights']);
        foreach (json_decode($row_lights['lights']) as $light) {
            update_light($light, json_encode($data));
            mysqli_query($con, 'UPDATE lights SET bri = bri + ' . $data['bri_inc'] . ' WHERE id = ' . $light . ';');
        }
    } else {
