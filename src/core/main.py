...
# If set to true all lights that unreachable are marked as off.
off_if_unreachable = False
            ...
            try:
                ...
                if bridge_config["schedules"][schedule]["status"] == "enabled":
                    if bridge_config["schedules"][schedule]["localtime"][-9:-8] == "A":
                        delay = random.randrange(0, int(bridge_config["schedules"][schedule]["localtime"][-8:-6]) * 3600 + int(
                            bridge_config["schedules"][schedule]["localtime"][-5:-3]) * 60 + int(bridge_config["schedules"][schedule]["localtime"][-2:]))
                    ...
                    if schedule_time.startswith("W"):
                        ...
                        if bridge_config["schedules"][schedule]["starttime"] <= (datetime.utcnow() - d).replace(microsecond=0).isoformat():
                            logging.info("execute timer: " +
                                         schedule + " withe delay " + str(delay))
                            bridge_config["schedules"][schedule]["starttime"] = datetime.utcnow().replace(
...
def updateAllLights():
    ...
    for light in bridge_config["lights_address"]:
        ...
        sendLightRequest(
            light, payload, bridge_config["lights"], bridge_config["lights_address"])
        sleep(0.5)
...
def daylightSensor():
    ...
    localzone = LocationInfo('localzone', bridge_config["config"]["timezone"].split("/")[1], bridge_config["config"]["timezone"], float(
        bridge_config["sensors"]["1"]["config"]["lat"][:-1]), float(bridge_config["sensors"]["1"]["config"]["long"][:-1]))
    ...
    deltaSunsetOffset = deltaSunset.total_seconds(
    ) + bridge_config["sensors"]["1"]["config"]["sunsetoffset"] * 60
    deltaSunriseOffset = deltaSunrise.total_seconds(
    ...
    if deltaSunriseOffset < 0 and deltaSunsetOffset > 0:
        bridge_config["sensors"]["1"]["state"] = {
            "daylight": True, "lastupdated": current_time.strftime("%Y-%m-%dT%H:%M:%S")}
        logging.info("set daylight sensor to true")
    else:
        bridge_config["sensors"]["1"]["state"] = {
            "daylight": False, "lastupdated": current_time.strftime("%Y-%m-%dT%H:%M:%S")}
...
if __name__ == "__main__":
    ...
    if "emulator" in bridge_config and "mqtt" in bridge_config["emulator"] and bridge_config["emulator"]["mqtt"]["enabled"]:
        mqtt.mqttServer(bridge_config["emulator"]["mqtt"], bridge_config["lights"],
                        bridge_config["lights_address"], bridge_config["sensors"])
    try:
        ...
        Thread(target=ssdpBroadcast, args=[
               HOST_IP, HOST_HTTP_PORT, mac]).start()
        Thread(target=scheduleProcessor).start()
        Thread(target=syncWithLights, args=[bridge_config["lights"], bridge_config["lights_address"],
                                            bridge_config["config"]["whitelist"], bridge_config["groups"], off_if_unreachable]).start()
        Thread(target=entertainmentService, args=[
               bridge_config["lights"], bridge_config["lights_address"], bridge_config["groups"], HOST_IP]).start()
        ...
        Thread(target=daylightSensor).start()
        Thread(target=api.remote.remoteApi, args=[
               BIND_IP, bridge_config["config"]]).start()
        if not configManager.runtimeConfig.arg["disableOnlineDiscover"]:
            Thread(target=api.remote.remoteDiscover,
                   args=[bridge_config["config"]]).start()
