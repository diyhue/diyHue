...
class FakeDevice:
    def __init__(self, identifier, reachable=True, candidate=True):
        self.id_v2 = identifier
        self.group_v1 = "lights"
        self.protocol = "mqtt"
        self.protocol_cfg = {
            "state_topic": f"zigbee2mqtt/{identifier}",
            "motion_aware_candidate": candidate,
        }
...
class MotionAwareRuntimeTests(unittest.TestCase):
    ...
    def test_false_and_zero_survive_update_and_resource_render(self):
        ...
        self.assertEqual(updated_service["sensitivity"]["status"], "set")
        self.assertFalse(
